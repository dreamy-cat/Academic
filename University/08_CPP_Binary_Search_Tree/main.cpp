...
//  Реализовать шаблонный класс потокобезопасного двоичного дерева (thread safe binary search tree) со следующими методами:
...
// проверить работу структуры на типах int, double, class c операторами сравнения;
// дерево должно использоваться несколькими потоками.
...
{   // Основной код, тестируем класс бинарного дерева поиска и выводим общую информацию.
    ...
    bstE.add(7);
    readingTask.get();  //  // if defered, sync
