...
void labs_03(void)
{
    ...
    int a, b, c, d, e, f, g;                                                    // Все необходимые переменные объвим сразу, лишнее можно уже удалить позже.
    a = 1; b = 0; c = 0;                                                        // По условию заданий, "ABC" или "XYZ" для всех используем один набор переменных.
    printf("\n3.1. A = %d, B = %d and C = %d as boolean type.\n", a, b, c);     // Простое задание, на разминку и знание теории.
    d = a || b;
    ...
    a = 2; b = 2; c = 2; d = 5;
    e = (a == c || b == d);
    printf("A) If rook at [%d,%d] is threat to position [%d,%d]. "              // Ладья атакует позицию.
        "Is (%d == %d or %d == %d) = %d.\n", a, b, c, d, a, c, b, d, e);
    a = 7; b = 6; c = 3; d = 2;
    e = (abs(c - a) == abs(d - b));
    printf("B) If bishop at [%d,%d] is threat to position [%d,%d]. "            // Слон атакует позицию.
           "Is(abs(% d - % d) == abs(% d - % d)) = % d.\n", a, b, c, d, c, a, d, b, e);
    a = 3; b = 5; c = 2; d = 4;
    e = (abs(c - a) <= 1 && abs(d - b) <= 1);
    printf("V) If king at [%d,%d] can move to position [%d,%d]. "               // Может-ли король двигаться на позицию.
           "Is abs(%d - %d) <= 1 and abs(%d - %d) = %d.\n", a, b, c, d, c, a, d, b, e);
    a = 7; b = 2; c = 2; d = 2;
    e = (a == c || b == d) || (abs(c - a) == abs(d - b));
    printf("G) If queen at [%d,%d] is threat to position [%d,%d]. "             // Ферзь атакует позицию.
    ...
    printf("Move: %d == %d or %d == %d = %d.\n", a, e, b, f, g);
    g &= !(e == c || f == d || abs(e - c) == abs(f - d));
    printf("Safe: and not (%d == %d or %d == %d or abs(%d - %d) == abs(%d - %d)) = %d.\n", e, c, f, d, e, c, f, d, g);
    a = 3; b = 2; c = 7; d = 7; e = 4; f = 4;
    printf("M) Knight at [%d,%d] move to [%d,%d] is safe from bishop at [%d,%d].\n", a, b, e, f, c, d);
    g = ((abs(e - a) == 1 && abs(f - b) == 2) || (abs(e - a) == 2 && abs(f - b) == 1));
    printf("Move: (abs(%d - %d) == 1 and abs(%d - %d) == 2) or (abs(%d - %d) == 2 and abs(%d - %d) == 1) = %d.\n",
        e, a, f, b, e, a, f, b, g);
    g &= !(abs(e - c) == abs(f - d));
    printf("Safe: move and not (abs(%d - %d) == abs(%d - %d)) = %d.\n", e, c, f, d, g);
    a = 5; b = 3; c = 7; d = 2; e = 6; f = 4;
    g = (abs(e - a) == 1 && abs(f - b) == 1);
    printf("U) King at [%d,%d] move to [%d,%d] is safe from knight at [%d,%d].\n", a, b, e, f, c, d);
    printf("Move: abs(%d - %d) == 1 and abs(%d - %d) == 1 = %d.\n", e, a, f, b, g);
    g &= (!((abs(e - a) == 1 && abs(f - b) == 2) || (abs(e - a) == 2 && abs(f - b) == 1)));
    printf("Safe: move and not ((abs(%d - %d) == 1 and abs(%d - %d) == 2) or (abs(%d - %d) == 2 and abs(%d - %d) == 1) = %d.\n",
        e, a, f, b, e, a, f, b, g);
}   
