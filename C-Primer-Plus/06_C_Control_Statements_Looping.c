...
unsigned short bits_max(unsigned short bits);				// Для демонстрации объявления и определения, после основного кода.

void part_06(void)
{
	printf("Part 6. C Control Statements: Looping.\n\n");
	// Основные структурные элементы для построения программы и дополнительно полнота по Тьюрингу.
	// Что такое цикл, оператор while и составной(сложный) оператор по предыдущей главе?
	// Какие есть виды циклов, с пред и пост-условием, с параметром, определенные и не определенные по итерациям?
	int a, b, c;
	a = 0; b = 3;
	printf("Simple loop using operator 'while':\n");
	while (a++ < 5) {										// Идиома с присвоением или изменением выражения на месте.
		printf("A: %d\tB: %d\tSum: %d\n", a, b, a + b);		// Попробовать сделать цикл бесконечным. Вниметельно с отступами.
	...
	// Сравнение: операции и выражения отношений, отметить про псевдокод.
	// Базовые типы и операции допустимые для выражения отношений?
	...
	// Не использовать оператор (не)равняется для чисел с плавающей точкой, только меньше или больше в некоторой эпсилон окрестности.
	double f = 1.0, g = 1.3;
	while (f != g && f < 1.5) {
	...
	// Вопрос: можно ли присваивать или использовать в арифметических выражениях логические результаты и кратку форму.
	...
	while (a--)												// Сравнение по-умолчанию. "До тех пор, пока не ноль.".
		printf("%d ", a);
	// Цикл со счётчиком или параметром. Выражения инициализация, сравнения и изменения.
	...
	// Вложенные циклы. Какие бывают и как соотносятся относительно друг-друга.
	printf("\n\nLoops in loop using for or while.\n");
	printf("\nMultiplication table.\n");
	for (int i = 1; i <= 9; ++i)
		printf("\t%d", i);
	for (int i = 1; i <= 9; ++i) {							// Внешний цикл относительно j.
		printf("\n%d\t", i);
		for (int j = 1; j < 9; ++j)							// Допустима модификация и зависимость 
			printf("%d\t", i * j);							// Внутренний цикл.
	}
	// Введение в массивы. Понимание что такое и как это представлено в памяти по индексу или смещению.
...
{	// Возвращаем максимальное количество комбинаций, представленное с помомщью bits бит(максимум 15).
	if (bits == 0 || bits > 0x0F) {							// Проверка на допустимость, вариант с условным оператором.
		printf("Bits is zero or more than max value 15, return 0.\n");
		return 0;
	}
	unsigned short r = 1;									// Один бит точно есть.
	// unsigned short r = (bits > 0);	Опасный код, т.к. 1 должна быть, но не гарантированно.
