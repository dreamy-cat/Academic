...
// Разобрать предыдущий черновик и объединить с предыдущим.
...
// Теоретический материал по книге Стивена Праты "Язык программирования Си, лекции и упражнения, 6-ое издание.".
// Для вещаний по повторению пройденной уже теории, закрепление и максимум быстрые сопутствующие вопросы.
// Все функции совпадают с главами книги и расчитаны на одно вещание примерно.

// Доклад ГербаСаттера. Про Бейсик 98-ого года и версию 6. Быстрый МФЦ.
// Простота команд, по Тьюрингу минимальный набор, но нужно быть осторожным чтобы не увлечься и не делать что-то сложное.
// Немного про Си, совместимость. Страуструп: "Деннис Ричи - это не могло быть сделано и он это сделал". Кроссплатформенность.
// Совместимость по исходному коду, и совместимость нового кода со старым кодом, незаметная сборка старорого кода.
// Пример с Питоном, когда к 20-ому году адаптация 3-его Питона заняла 12 лет, 90% стало на 3-ей, и не сломанная совместимость могла тут помочь,
// чтобы люди пробовали новые возможности понемногу, в окружении старого кода.
// https://youtu.be/BF3qw1ObUyo.

// Доклад старины Страуструпа. Две строчки про С++, суть: прямое отображение близко к железу и абстракции высокого уровня с разной стоимостью.
